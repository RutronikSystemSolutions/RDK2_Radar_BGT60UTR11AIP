                                               ...
                                               uint32_t len)
{
    ...
    if (CY_SCB_SPI_SUCCESS == status)
    {
    	...
    	for(;;)
    	{
    		uint32_t transfer_status = Cy_SCB_SPI_GetTransferStatus(mtb_iface->spi->base, &(mtb_iface->spi->context));
    		if ((CY_SCB_SPI_TRANSFER_ACTIVE & transfer_status) == 0)
    		{
    			// Transfer completed
    			break;
    		}
    		...
    		Cy_SysLib_DelayUs(1);
    		timeout_counter++;
    		if (timeout_counter > SPI_TRANSFER_TIMEOUT_US)
    		{
    			Cy_SCB_SPI_AbortTransfer(mtb_iface->spi->base, &(mtb_iface->spi->context));
...
//                                                        &(mtb_iface->spi->context));
        ...
        uint32_t data_len = (len * 12)/8;
        cy_en_scb_spi_status_t status = Cy_SCB_SPI_Transfer(mtb_iface->spi->base, NULL, rx_data, data_len,
                                                                &(mtb_iface->spi->context));
    if (CY_SCB_SPI_SUCCESS == status)
    {
    	uint32_t timeout_counter = 0;
